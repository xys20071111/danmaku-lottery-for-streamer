	...
	public async connect() {
		...
		request.on('response', (response) => {
			...
			response.on('end', () => {
				...
				// 连接弹幕服务器
				this.socket = new WebSocket(`wss://${parsedData.data.host_list[0].host}:${parsedData.data.host_list[0].wss_port}/sub`)
				...
				this.socket.on('open', async () => {
					...
					const data = JSON.stringify({
						roomid: parseInt(this.roomId.toString(), 10), protover: 3, platform: 'web', uid: 0, key: parsedData.data.token,
					})
